...
def test_two_videos_concatenation(sample_videos, mock_ffmpeg_probe, mock_ffmpeg_run):
    """2つの動画を正常に連結できるかテスト"""
    ...
    result = (
        movie(video1)
        .append(video2, duration=1.0, effect=CrossfadeEffect.FADE, mode=TransitionMode.CROSSFADE_NO_INCREASE)
...
def test_three_videos_concatenation(sample_videos, mock_ffmpeg_probe, mock_ffmpeg_run):
    """3つの動画を正常に連結できるかテスト"""
    ...
    result = (
        movie(video1)
        .append(video2, duration=1.0, effect=CrossfadeEffect.WIPELEFT, mode=TransitionMode.CROSSFADE_INCREASE)
        .append(video3, duration=1.5, effect=CrossfadeEffect.DISSOLVE, mode=TransitionMode.CROSSFADE_NO_INCREASE)
        .execute(output_path, quiet=True)
    )
...
def test_file_not_found_error(mock_ffmpeg_probe):
...
def test_single_video_error(mock_ffmpeg_probe, mock_ffmpeg_run):
...
def test_method_chaining_returns_self(sample_videos, mock_ffmpeg_probe, mock_ffmpeg_run):
    """appendメソッドが自身のインスタンスを返すかテスト"""
    video1, video2 = sample_videos[:2]
    sequence = movie(video1)
    chained_sequence = sequence.append(video2, mode=TransitionMode.CROSSFADE_INCREASE)
    assert sequence is chained_sequence



def test_increase_mode_concatenation(sample_videos, mock_ffmpeg_probe, mock_ffmpeg_run):
    ...
    result = (
        movie(video1)
        .append(video2, duration=transition_duration, effect=CrossfadeEffect.DISSOLVE, mode=TransitionMode.CROSSFADE_INCREASE)
        .execute(output_path, quiet=True)
    )
